...
La alternativa «Voz del navegador» usa reconocimiento y síntesis del navegador con el mismo transporte global autorizado; no utiliza el endpoint empresarial del modo compatible antiguo. Requiere soporte de reconocimiento y síntesis en el navegador. Se inicia expresamente con «Hablar» y pausa tanto la captura como la consulta y reproducción; el proveedor del navegador puede procesar el audio. La calidad y disponibilidad de esta voz dependen del dispositivo. Las consultas siguen sujetas a los límites normales del chat global.

## Consola del fundador: administración de empresas

`/overview` pasa a ser el centro de mando para quien funda empresas. Por cada empresa propia muestra salud (una señal: cobro, cupo, rutinas con fallos, bloqueos o asientos llenos), plan y asientos usados, respuestas del mes frente a su límite, personas e invitaciones pendientes, integraciones conectadas, rutinas activas y ejecuciones con error en 7 días, última actividad de auditoría y los cuatro pendientes de siempre. Hay vista de tarjetas y de tabla (se recuerda por navegador), un filtro por grupo empresarial y el tope de empresas propias (N de 5). Las empresas donde la cuenta es gerente o colaboradora siguen apareciendo solo con sus pendientes: su administración es de su fundador.

- **Personas** (`/overview/people`): una fila por persona de todas las empresas propias, con su rol y fecha de ingreso en cada una, además de las invitaciones pendientes. Permite invitar un correo a una o varias empresas a la vez, con el resultado de cada empresa por separado (cada una respeta su plan y sus asientos); cambiar el rol (miembro, admin de equipo o admin de la empresa); retirar a alguien, con confirmación que explica la salida segura de la 0138, y cancelar invitaciones.
- **Ficha de empresa** (`/overview/companies/[id]`): salud, plan y consumo, equipo con las mismas acciones, renombrar la empresa (el slug no cambia) y accesos directos a Personas, Plan, Integraciones, Auditoría, Actividad y Rutinas de esa empresa mediante `?workspace=`.
- **Grupos**: además de crear y mover empresas, se pueden renombrar y borrar. Al borrar un grupo, sus empresas quedan sin grupo y no se modifican.
- **Personas de cada empresa** (`/admin/users`): el cambio de rol ahora persiste. Antes solo escribía `public.users.role` y `resolveSessionDirectory` lo recalculaba desde `ba_member.role` en la petición siguiente. Ahora escribe primero la membresía en better-auth y después el directorio. También se puede retirar a alguien desde esa pantalla.

Reglas (una sola implementación en `lib/founder-rules.ts`, compartida por ambas pantallas): toda acción vuelve a comprobar en el servidor la propiedad de cada empresa afectada (`lib/founder-guard.ts`); un administrador no puede cambiar ni retirar a un fundador; ninguna empresa puede quedarse sin fundador; nadie cambia su propio rol desde estas pantallas; los espacios personales no tienen equipo. `/overview/*` y `/api/founder/*` son rutas globales: no heredan la empresa de la pestaña.

Límites: la transferencia de propiedad y el nombramiento de nuevos fundadores siguen fuera de alcance. Invitar a varias empresas no es atómico: cada invitación se envía y se informa por separado. Las cifras de salud son lecturas en vivo por empresa (sin caché); con cinco empresas son unas treinta consultas por carga. La última actividad sale de `audit_events` y no cuenta conversaciones sin herramientas. Integraciones cuenta proveedores OAuth distintos y no incluye servidores MCP. No se validó contra better-auth y Postgres reales: las pruebas usan dobles.
