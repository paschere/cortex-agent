...
describe("rate.estimate", () => {
  ...
  it("network error: throws IntegrationError when estimator returns non-ok", async () => {
    ...
    const ctx = makeCtx();
    await expect(runTool(rateEstimate, validInput, ctx)).rejects.toBeInstanceOf(
      IntegrationError,
    );
  });
...
describe("rate.estimate_from_document", () => {
  ...
  it("uses defaults when extraction fails for role", async () => {
    ...
    const result = await runTool(
      ...
      {
        documentText:
          "We need a senior engineer in br with 3 years of experience.",
  ...
  it("throws ValidationError when seniority cannot be extracted and no default provided", async () => {
    ...
    await expect(
      runTool(
        ...
        {
          documentText:
            "Looking for a backend engineer with 5 years of experience.",
        },
        ctx,
      ),
    ).rejects.toBeInstanceOf(ValidationError);
  ...
  it("defaults region to latam when not found in text", async () => {
    ...
    const result = await runTool(
      ...
      {
        documentText:
          "We need a senior backend engineer with 3 years of experience.",
      },
      ctx,
    );
  ...
  it("network error from estimator surfaces as IntegrationError", async () => {
    ...
    await expect(
      runTool(
        ...
        {
          documentText:
            "We need a senior backend developer in mx with 5 years of experience.",
        },
        ctx,
      ),
