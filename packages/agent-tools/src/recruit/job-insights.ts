...
export const jobInsights = registerTool({
  ...
  description:
    ...
    'PROVENANCE: the totals and stage counts come from the Zipdev matcher DB; every score, ranking and "success probability" is Zipdev AI scoring; interview figures come from AI interview analysis and test figures from TestGorilla. Attribute each to its own system — none of it is Workable ATS data or client feedback — and cite meta.fetchedAt for freshness.',
  ...
  handler: async (input) => {
    ...
    if (top.length) {
      lines.push('');
      lines.push('**Top candidates** (Zipdev AI scoring):');
