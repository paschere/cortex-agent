...
function renderMarkdown(
  ...
  if (stages.length === 0) {
    return [
      `No client pipeline stages for ${jobLabel}.`,
      "",
...
export const pipelineKanban = registerTool({
  ...
  handler: async (input) => {
    ...
    const data = await matcherFetch(
      `/api/jobs/${encodeURIComponent(input.jobId)}/kanban`,
    );
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    const stages = rawStages.map((s) => ({
      ...
      candidates: (s?.presentations ?? []).map((p: any) => ({
        ...
        name:
          [p?.candidateFirstName, p?.candidateLastName]
            .filter(Boolean)
            .join(" ") || "(candidate)",
