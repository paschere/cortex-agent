...
function renderMarkdown(candidates: any[], jobLabel: string, meta: ToolMeta): string {
  if (candidates.length === 0) {
    return [`No candidates matched for ${jobLabel}.`, '', provenanceFooter(meta)].join('\n');
  }
  const lines: string[] = [];
  lines.push(`**${candidates.length} candidate(s) for ${jobLabel}** (ranked by AI match score)`);
  lines.push('');
  lines.push('| # | Candidate | Stage | Score | Exp | Interviews | Ratings | TestGorilla |');
  lines.push('| --- | --- | --- | --- | --- | --- | --- | --- |');
...
export const listCandidates = registerTool({
  ...
  description:
    ...
    'PROVENANCE: `scores.*` and `summary` are Zipdev AI output — attribute them that way and never as Workable ATS data. `signals.*` name their own systems (interview analysis, recruiter ratings, TestGorilla), and `source` / `links` show where each person came from and where to verify them. Report the pool size and the freshness alongside any ranking you give.',
  ...
  handler: async (input) => {
    ...
    if (lean.available) {
      const meta = metaFromServer(lean.data.meta, '/api/internal/recruit/candidates');
    ...
    if (input.minScore != null) {
      raw = raw.filter((c) => typeof c?.matchScore === 'number' && c.matchScore >= input.minScore!);
    }
    const totalAvailable = raw.length;
    const page = raw.slice(offset, offset + limit).map((c) => candidateFromLegacy(c, input.jobId));
    ...
    const meta = buildMeta({
      ...
      provenance: {
        'name, email, topSkills, experienceYears':
          'Workable ATS / Zipdev matcher DB — imported profile data',
        'stage, appliedAt': 'Zipdev matcher DB — pipeline state maintained by recruiters',
        'scores.*, summary': 'Zipdev AI scoring — derived, never an ATS field',
      },
      dataQuality: [
