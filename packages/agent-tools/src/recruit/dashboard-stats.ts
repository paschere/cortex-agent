...
export const dashboardStats = registerTool({
  ...
  description:
    ...
    'PROVENANCE: counts come from the Zipdev matcher DB; "AI accuracy" and "interview success" are Zipdev-computed metrics, not externally validated benchmarks — present them as internal figures and cite meta.fetchedAt.',
  ...
  handler: async () => {
    ...
    const meta = buildMeta({
      ...
      provenance: {
        ...
        'metrics.aiAccuracy': `${SOURCE.aiScoring} — Zipdev's own accuracy metric, not an external benchmark`,
    ...
    if (m) {
      ...
      if (m.aiAccuracy) parts.push(`- AI accuracy (Zipdev-computed): ${m.aiAccuracy.percentage}%`);
