...
function renderMarkdown(c: any, meta: ToolMeta): string {
  ...
  if (apps.length) {
    lines.push('');
    lines.push(`**Applications (${apps.length})** — scores are Zipdev AI scoring, not ATS data:`);
...
export const getCandidate = registerTool({
  ...
  description:
    ...
    'PROVENANCE: profile fields come from the candidate record imported into the Zipdev matcher (Workable ATS where workableId is set); every score and summary is Zipdev AI scoring and must be attributed as such, never as ATS or client feedback. `source.profileUpdatedAt` tells you how fresh the profile is — cite it.',
  ...
  handler: async (input) => {
    const raw = await matcherFetch(`/api/candidates/${encodeURIComponent(input.candidateId)}`);

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const rawApps: any[] = Array.isArray(raw?.applications) ? raw.applications : [];
    const applications = rawApps.map((a) => {
      const summary = shortSummary(a?.insights?.executiveSummary ?? a?.llmRationale, 280);
      return {
        applicationId: a?.id ?? null,
        jobId: a?.jobId ?? null,
        jobTitle: a?.jobTitle ?? null,
        // /api/candidates/:id defaults an unlinked job's company to "Zipdev";
        // treat that literal as "unknown" rather than repeating it as a client.
        client: a?.company && a.company !== 'Zipdev' ? a.company : null,
        stage: a?.status ?? null,
        ...
        scoreHistoryPoints: Array.isArray(a?.scoreHistory) ? a.scoreHistory.length : 0,
        latestScoreAt: a?.scoreHistory?.[0]?.calculatedAt ?? null,
        recruiterRatings: Array.isArray(a?.recruiterRatings) ? a.recruiterRatings.length : 0,
        lastActivityAt: a?.scoreHistory?.[0]?.calculatedAt ?? null,
        scoreSource: SOURCE.aiScoring,
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...(input.includeRawInsights ? { rawInsights: a?.insights ?? null } : {}),
    ...
    const candidate = {
      ...
      skills: Array.isArray(raw?.skills)
        ...
        : [],
      applications,
      recentActivityCount: Array.isArray(raw?.recentActivity) ? raw.recentActivity.length : 0,
      resumeChars: typeof raw?.resumeText === 'string' ? raw.resumeText.length : 0,
      ...(input.includeResumeText ? { resumeText: raw?.resumeText ?? null } : {}),
      source: {
      ...
      links: { matcher: matcherLink(`/candidates/${raw?.id ?? input.candidateId}`) },
    };
    ...
    const meta = buildMeta({
      ...
      dataQuality: [
        ...
        ...(applications.some((a) => a.client === null)
          ? [
              'Some applications show no client because their job is not linked to a company in the matcher (the underlying API reports "Zipdev" there, which is a placeholder).',
            ]
          : []),
      ],
    });
