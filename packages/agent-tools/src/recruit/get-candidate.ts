...
function renderMarkdown(c: any, meta: ToolMeta): string {
  ...
  if (apps.length) {
    lines.push('');
    lines.push(`**Applications (${apps.length})** — scores are Cortex AI scoring, not ATS data:`);
    ...
    const meta = buildMeta({
      ...
      dataQuality: [
        ...
        ...(applications.some((a) => a.client === null)
          ? [
              'Some applications show no client because their job is not linked to a company in the matcher (the underlying API reports a placeholder name there).',
            ]
          : []),
      ],
    });
