...
export const findMatches = registerTool({
  ...
  description:
    "Kick off DEEP candidate matching for a requisition: starts an async run that scores the talent pool by skills, embedding similarity and cross-job signals, then LLM-evaluates the top ~50 and attaches qualifiers to the job — it costs minutes. Results are NOT returned by this call — immediately follow up with recruit.wait_for_matches, which polls the run's real status server-side and returns the pool when it is genuinely finished. For the everyday 'who are the top N for this job and why' question, use workable.top_candidates instead — it ranks the job's live Workable pipeline in seconds. Use THIS tool when the user wants a fresh, thorough evaluation or wants recommended candidates actually added to the job's pipeline. " +
    "PROVENANCE: everything this run produces is Cortex AI scoring — derived signal, not Workable ATS data or client feedback. Say so when you present the shortlist. " +
  ...
  handler: async (input) => {
    const data = await matcherFetch(
      `/api/jobs/${encodeURIComponent(input.jobId)}/find-matches`,
      {
        method: "POST",
      },
    );
