...
export const scoreCandidate = registerTool({
  ...
  description:
    ...
    'PROVENANCE: the score itself is Zipdev AI scoring, but `contributions` names the real underlying systems it was built from. Report it as an AI assessment, name the inputs, and cite `scoredAt` — a score computed weeks ago before an interview happened is stale, and meta.dataQuality will say so.',
  ...
  handler: async (input) => {
    if (input.jobId) {
      ...
      const summary = shortSummary(latest?.executiveSummary ?? latest?.rationale, 600);
    ...
    const data = await matcherFetch(`/api/candidates/${encodeURIComponent(input.candidateId)}`);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const apps: any[] = Array.isArray(data?.applications) ? data.applications : [];
    const name =
      data?.name ??
      (`${data?.firstName ?? ''} ${data?.lastName ?? ''}`.trim() || input.candidateId);
