...
function renderMarkdown(r: any, meta: ToolMeta): string {
  ...
  if (Array.isArray(r.topCandidates) && r.topCandidates.length) {
    lines.push("");
    lines.push(
      "**Top-scored candidates** (Cortex AI scoring, not an ATS ranking):",
    );
  ...
  handler: async (input) => {
    ...
    if (lean.available) {
      const meta = metaFromServer(
        lean.data.meta,
        `/api/internal/recruit/requisitions/${input.id}`,
      );
      return {
    ...
    const truncated =
      !input.includeFullDescription && raw.length > DESCRIPTION_CAP;
    const requisition = {
      ...projected,
      employmentType: job?.type ?? null,
      salary: job?.salary ?? null,
      requirements: job?.requirements ?? null,
      description: input.includeFullDescription
        ? raw
        : raw.slice(0, DESCRIPTION_CAP),
    ...
    const meta = buildMeta({
      ...
      dataQuality: [
        ...
        ...(projected.clientAttribution === "unlinked"
          ? [
              'This requisition has no company linked, so `client` is null. The legacy endpoint would have said "Cortex" — that is a placeholder, not a client.',
            ]
          : []),
        ...(truncated
          ? [
