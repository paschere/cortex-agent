...
function renderMarkdown(r: any, meta: ToolMeta): string {
  ...
  if (Array.isArray(r.topCandidates) && r.topCandidates.length) {
    lines.push('');
    lines.push('**Top-scored candidates** (Zipdev AI scoring, not an ATS ranking):');
...
export const getRequisition = registerTool({
  ...
  description:
    ...
    'PROVENANCE: `source` states whether the role came from Workable ATS or was created in the Zipdev matcher, with syncedAt/lastUpdatedAt, and `links` gives the matcher and Workable URLs. Cite the system and freshness when you report, and label topCandidates[].score as Zipdev AI scoring — it is not an ATS field. Relay meta.dataQuality (unlinked client, unset status, truncation) instead of papering over it.',
  ...
  handler: async (input) => {
    ...
    const meta = buildMeta({
      ...
      provenance: {
        ...
        'client, owner, budget': 'Zipdev matcher DB',
        'candidates.*': 'Zipdev matcher DB — counts as reported by /api/jobs',
      },
      dataQuality: [
        ...
        ...(projected.clientAttribution === 'unlinked'
          ? [
              'This requisition has no company linked, so `client` is null. The legacy endpoint would have said "Zipdev" — that is a placeholder, not a client.',
            ]
          : []),
        ...(truncated
