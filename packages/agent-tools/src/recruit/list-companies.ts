...
function renderMarkdown(companies: any[], meta: ToolMeta): string {
  if (companies.length === 0) return ['No clients matched.', '', provenanceFooter(meta)].join('\n');
  const lines: string[] = [];
  lines.push(`**${companies.length} client(s)**`);
  lines.push('');
  lines.push('| Client | Industry | Requisitions (open/total) | Candidates in pipelines |');
  lines.push('| --- | --- | --- | --- |');
...
export const listCompanies = registerTool({
  ...
  description:
    'List Zipdev clients from the recruitment system, each with how many requisitions they have, how many are still open, and how many candidates sit in their pipelines — enough to answer "which clients are we actively hiring for" in one call. ' +
    ...
    'PROVENANCE: client records live in the Zipdev matcher DB (not in Workable), and every count is computed live from the matcher. Read meta.dataQuality out loud when it matters: many requisitions are not linked to any client, so per-client totals are a floor, not a census.',
  ...
  handler: async (input) => {
    ...
    if (lean.available) {
      const meta = metaFromServer(lean.data.meta, '/api/internal/recruit/companies');
      return {
    ...
    const companies = rows.slice(offset, offset + limit).map((c) => ({
      ...
      isActive: true,
      requisitions: { total: null, openNotClosed: null, candidatesInPipelines: null },
      source: { origin: SOURCE.matcher, readFrom: SOURCE.matcher, lastUpdatedAt: null },
      links: { matcher: c?.id ? matcherLink(`/positions?companyId=${c.id}`) : null },
    }));
