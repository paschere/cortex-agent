...
describe('requisitionFromLegacyJob', () => {
  const legacy = {
    ...
    // /api/jobs substitutes "Zipdev" whenever the job has no company linked.
    company: 'Zipdev',
    companyId: null,
  ...
  it('refuses the "Zipdev" placeholder when no company is linked', () => {
  ...
  it('carries provenance and verifiable links', () => {
    ...
    expect(r.links.workable).toBe('https://zipdev.workable.com/backend/jobs/7FE1AB41FC');
...
describe('candidateFromLegacy', () => {
  it('drops resume text and raw analysis, keeping the decision fields', () => {
    ...
    expect(c.scores.source).toBe('Zipdev AI scoring');
...
describe('internalFetch', () => {
  ...
  it('throws on a JSON 404 — "not found" is a real answer, not a reason to fall back', async () => {
    globalThis.fetch = vi
      .fn()
      .mockResolvedValue(
        new Response(JSON.stringify({ error: 'Requisition not found' }), { status: 404 }),
      ) as unknown as typeof fetch;
