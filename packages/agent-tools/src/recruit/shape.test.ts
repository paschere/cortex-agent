...
describe("requisitionFromLegacyJob", () => {
  const legacy = {
    ...
    // /api/jobs substitutes "Cortex" whenever the job has no company linked.
    company: "Cortex",
    companyId: null,
  ...
  it('refuses the "Cortex" placeholder when no company is linked', () => {
...
describe("internalFetch", () => {
  ...
  beforeEach(() => {
    process.env.Cortex_MATCHER_URL = "https://matcher.test";
  });
  afterEach(() => {
  ...
  it('throws on a JSON 404 — "not found" is a real answer, not a reason to fall back', async () => {
    ...
    await expect(
      internalFetch("/api/internal/recruit/requisitions/nope"),
  ...
  it("returns the payload on success", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({ requisitions: [], meta: { fetchedAt: "now" } }),
        {
          status: 200,
        },
      ),
