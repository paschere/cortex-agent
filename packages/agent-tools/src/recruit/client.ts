export const BASE = () => process.env.ZIPDEV_MATCHER_URL ?? 'http://localhost:3100';
...
export async function matcherFetch(path: string, init?: RequestInit): Promise<any> {
  ...
  for (let attempt = 0; attempt <= RETRIES; attempt++) {
    ...
    try {
      ...
      if (
        lastError.message.includes('zipdev-matcher') &&
        !/zipdev-matcher 5\d\d/.test(lastError.message)
      ) {
        throw lastError;
      }
    }
...
export async function internalFetch<T>(
  ...
  for (let attempt = 0; attempt <= RETRIES; attempt++) {
    ...
    if (res.status === 401 || res.status === 403) {
      return {
        available: false,
        reason: 'ZIPDEV_MATCHER_TOKEN is not set, or the matcher rejected it',
      };
    }
    if (res.status === 404 && !isJson) {
      return { available: false, reason: 'lean internal endpoint is not deployed on this matcher' };
    }
    if (res.status < 500) {
