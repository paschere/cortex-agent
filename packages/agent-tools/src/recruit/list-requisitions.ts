...
export const listRequisitions = registerTool({
  ...
  description:
    ...
    'Relay meta.dataQuality when it matters — in particular, most requisitions have no client linked, so `client` is null instead of a misleading "Zipdev".',
  ...
  handler: async (input) => {
    ...
    const meta = buildMeta({
      ...
      provenance: {
        ...
        'client, owner': 'Zipdev matcher DB',
      ...
      dataQuality: [
        ...
        ...(unlinked
          ? [
              `Client attribution is incomplete: ${unlinked} of ${page.length} returned requisitions have no company linked, so their client is null. Do not report them as Zipdev's own roles.`,
            ]
          : []),
      ],
    });
