...
function renderMarkdown(reqs: any[], meta: ToolMeta): string {
  if (reqs.length === 0)
    return ["No requisitions matched.", "", provenanceFooter(meta)].join("\n");
...
export const listRequisitions = registerTool({
  ...
  inputSchema: z.object({
    ...
    sort: z
      .enum(["lastActivity", "daysOpen", "candidates", "title"])
  ...
  handler: async (input) => {
    ...
    if (input.ownerId) {
      jobs = jobs.filter(
        (j) =>
          j?.recruiterId === input.ownerId || j?.sourcerId === input.ownerId,
      );
    }
    if (input.search) {
    ...
    const page = jobs
      ...
      .map(requisitionFromLegacyJob);
    const unlinked = page.filter(
      (r) => r.clientAttribution === "unlinked",
    ).length;
