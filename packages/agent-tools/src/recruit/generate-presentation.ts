...
export const generatePresentation = registerTool({
  ...
  description:
    "Generate an AI-written candidate presentation (formatted HTML) from the candidate's skills, experience, education and latest score insights. This CREATES/updates a stored presentation (version-bumped, createdBy='AI') and therefore requires confirmation. It produces no file and nothing anyone can be sent — if the user wants something to hand to a client, use presentations.create_pdf, which writes the draft itself when there is not one yet and comes back with a download link. Use THIS tool only to refresh the stored draft on its own; to just read an existing one, use recruit.get_presentation. " +
  ...
  handler: async (input) => {
    ...
    const htmlChars = typeof p?.htmlContent === 'string' ? p.htmlContent.length : 0;
