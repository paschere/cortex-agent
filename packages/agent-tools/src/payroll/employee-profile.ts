...
export const payrollEmployeeProfile = registerTool({
  ...
  description:
    'Get the full picture of one person on the Zipdev team: who they are, which client they are placed with, their job title, when they started and how long they have been here, their current pay rate and how it changed over time, what they were paid in recent payroll periods, and their expense history. ' +
  ...
  inputSchema: z.object({
    ...
    periods: z
      ...
      .max(24)
      .optional()
      .describe('How many recent completed payroll periods to include (default 8)'),
  }),
  ...
  handler: async (input, ctx) => {
    ...
    const lines: string[] = [];
    lines.push(`**${p?.name ?? 'Unknown'}** — ${p?.jobTitle ?? 'no title on file'}`);
    lines.push('');
    lines.push(
      ...
      if (ch.length > 1 && first && last) {
        const delta =
          first.basePayRateUsd > 0
            ? Math.round(
                ((last.basePayRateUsd - first.basePayRateUsd) / first.basePayRateUsd) * 100,
              )
            : 0;
        lines.push(
    ...
    const cats = exp && Array.isArray(exp.byCategory) ? exp.byCategory.slice(0, 5) : [];
    if (cats.length) {
      lines.push('');
      lines.push(
    ...
    if (typeof data.monthlyReviewCount === 'number' && data.monthlyReviewCount > 0) {
      lines.push('');
      lines.push(`_${data.monthlyReviewCount} monthly client reviews on file._`);
    }

    lines.push('');
    lines.push('_Compensation figures are confidential — share only with the person asking, never externally without explicit confirmation._');
