...
export const payrollTeamAssignments = registerTool({
  ...
  description:
    ...
    'It always carries the monthly cost, so it is a compensation read even when the question was only "who is on X" — treat it as such. ' +
    `${PAYROLL_SOURCE_NOTE} ` +
  ...
  inputSchema: z.object({
    ...
    q: z.string().min(1).optional().describe('Free-text search over name, email and job title'),
  ...
  handler: async (input, ctx) => {
    ...
    lines.push('');
    lines.push(
      '_Compensation figures are confidential — do not share externally without explicit confirmation._',
    );
