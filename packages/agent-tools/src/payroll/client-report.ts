...
export const payrollClientReport = registerTool({
  ...
  inputSchema: z.object({
    client: z
      ...
      .min(2)
      .describe(
        'Client name as it appears in payroll (the "department" field)',
      ),
  ...
  handler: async (input, ctx) => {
    ...
    const [expenses, stats] = await Promise.all([
      fetchExpensesReport(ctx, { months, client: input.client }).catch(
        () => null,
      ),
      fetchPayrollStats(ctx, { periods, client: input.client }).catch(
        () => null,
      ),
    ]);
    ...
    if (!e) {
      ...
      if (byCat.length > 0) {
        ...
        for (const c of byCat.slice(0, 8)) {
          lines.push(
            `| ${c.category ?? "—"} | ${money(c.totalUsd as number)} |`,
          );
        }
      }
    }
