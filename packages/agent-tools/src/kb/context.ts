...
export const kbContext = registerTool({
  id: 'kb.context',
  description:
    "Build a grounded context briefing on a topic from Zipdev's Knowledge Base. Give it the topic (and optionally a few related angles) and it searches the brain from every angle, removes duplicates, groups the findings per source document, and returns a citation-ready block plus the structured sources. Use this before writing anything that should reflect what Zipdev already knows — a proposal, a client email, a rate answer, an internal explanation — instead of running several separate searches. If it returns nothing, say plainly that the Knowledge Base has no material on the topic rather than inventing an answer.",
