...
const BASE = 'https://api.bamboohr.com/api/gateway.php/zipdev/v1';
...
const ROWS = [
  {
    ...
    workEmail: 'emmanuel.castro@zipdev.com',
    ...
    supervisorEmail: 'adriana.garcia@zipdev.com',
    payRate: '4500.00 USD',
  ...
  },
  {
    ...
    workEmail: 'adriana.garcia@zipdev.com',
  ...
  },
  {
    ...
    workEmail: 'jazmin.montoya@zipdev.com',
    jobTitle: 'Recruiter',
    department: 'Internal',
    division: 'Zipdev',
    location: 'Mexico',
...
const server = setupServer(
  ...
  http.get(WHOS_OUT, () =>
    HttpResponse.json([
      ...
      { id: 2, type: 'holiday', name: 'Independence Day', start: '2026-09-16', end: '2026-09-16' },
    ]),
  ),
  http.get(REQUESTS, () =>
    HttpResponse.json([
  ...
  http.get(CHANGED, () =>
    HttpResponse.json({
      ...
      employees: {
        '174': { id: '174', action: 'Updated', lastChanged: '2026-07-22T15:46:43+00:00' },
        '999': { id: '999', action: 'Deleted', lastChanged: '2026-07-25T10:00:00+00:00' },
      },
    }),
  ),
  http.get(ENTRIES, () =>
    HttpResponse.json([
      {
        ...
        note: null,
        approved: true,
        projectInfo: { project: { id: 11, name: 'Click Call Sell' }, task: null },
      },
      {
        id: 2,
        employeeId: 174,
        ...
        approved: false,
        projectInfo: { project: { id: 11, name: 'Click Call Sell' }, task: { name: 'Meetings' } },
      },
    ]),
  ),
...
describe('BambooHR tools — successful parses', () => {
  ...
  it('the employee lookup keeps pay and bill rate apart and works out the margin', async () => {
    ...
    expect(out.guidance).toMatch(/what Zipdev pays/i);
    expect(out.guidance).toMatch(/what Zipdev charges/i);
  ...
  it('withholds compensation when it is not asked for', async () => {
    const out = await bambooGetEmployee.handler(
      { email: 'emmanuel.castro@zipdev.com', includeCompensation: false },
      fakeCtx(),
    );
    expect(out.found).toBe(true);
  ...
  it('prefers the person who still works here when a name collides', async () => {
    server.use(
      http.post(REPORTS, () =>
        HttpResponse.json({
          employees: [
            { ...ROWS[0], id: '1', displayName: 'Ana Ruiz', status: 'Inactive' },
  ...
  it('the timesheet summary aggregates rather than handing over raw entries', async () => {
    ...
    expect(out.byProject[0]).toEqual({ project: 'Click Call Sell', hours: 7.75, people: 1 });
  ...
  it('individual entries are scoped to one person', async () => {
    const out = await bambooTimesheetEntries.handler(
      { name: 'Emmanuel Castro', start: '2026-07-01', end: '2026-07-14', limit: 100 },
      fakeCtx(),
    );
    expect(out.found).toBe(true);
  ...
  it('the field catalogue finds bill rate and names the restricted fields', async () => {
    ...
    // Bill Rate has no alias — it is one of Zipdev's custom fields.
  ...
  it('a missing bill-rate table is treated as no data, not as a failure', async () => {
    server.use(
      http.get(TABLE, ({ params }) => {
        ...
        return HttpResponse.json([
          { id: '1', startDate: '2024-04-01', rate: { currency: 'USD', value: '4000.00' } },
        ]);
      }),
    );
    const out = await bambooCompensationHistory.handler({ name: 'Emmanuel Castro' }, fakeCtx());
