...
const BASE = "https://api.bamboohr.com/api/gateway.php/Cortex/v1";
...
const ROWS = [
  {
    ...
    workEmail: "emmanuel.castro@Cortex.com",
    ...
    supervisorEmail: "adriana.garcia@Cortex.com",
    payRate: "4500.00 USD",
  ...
  },
  {
    ...
    workEmail: "adriana.garcia@Cortex.com",
  ...
  },
  {
    ...
    workEmail: "jazmin.montoya@Cortex.com",
    jobTitle: "Recruiter",
    department: "Internal",
    division: "Cortex",
    location: "Mexico",
...
const server = setupServer(
  http.post(REPORTS, () =>
    HttpResponse.json({ title: "Report", fields: [], employees: ROWS }),
  ),
...
describe("BambooHR tools — not configured", () => {
  ...
  it("the roster reports itself unconfigured instead of throwing", async () => {
    const out = await bambooListEmployees.handler(
  ...
  it("every tool fails soft with a sentence a non-technical person can read", async () => {
    const results = await Promise.all([
      ...
      bambooGetEmployee.handler(
        { name: "Emmanuel", includeCompensation: true },
        fakeCtx(),
      ),
      ...
      bambooOrgChart.handler(
        { name: "Emmanuel", includeInactive: false },
        fakeCtx(),
      ),
      bambooHeadcount.handler(
        { groupBy: "division", status: "active", limit: 30 },
        fakeCtx(),
      ),
      bambooRecentlyChanged.handler(
      ...
      bambooTimesheetEntries.handler(
        { name: "Emmanuel", limit: 100 },
        fakeCtx(),
      ),
      bambooEmployeeProjects.handler({ name: "Emmanuel" }, fakeCtx()),
      bambooListDocuments.handler(
        { name: "Emmanuel", payslipsOnly: false, limit: 50 },
        fakeCtx(),
      ),
      bambooDescribeFields.handler(
        { includeTables: true, limit: 80 },
        fakeCtx(),
      ),
    ]);
...
describe("BambooHR tools — successful parses", () => {
  ...
  it("the employee lookup keeps pay and bill rate apart and works out the margin", async () => {
    ...
    expect(out.guidance).toMatch(/what Cortex pays/i);
    expect(out.guidance).toMatch(/what Cortex charges/i);
  ...
  it("withholds compensation when it is not asked for", async () => {
    const out = await bambooGetEmployee.handler(
      { email: "emmanuel.castro@Cortex.com", includeCompensation: false },
      fakeCtx(),
    );
    expect(out.found).toBe(true);
  ...
  it("the rate history interleaves pay and bill changes and finds the last raise", async () => {
    const out = await bambooCompensationHistory.handler(
      { name: "Emmanuel Castro" },
      fakeCtx(),
    );
    expect(out.found).toBe(true);
    ...
    const raise = out.changes.find(
      (c) => c.kind === "pay" && c.effectiveDate === "2025-10-01",
    );
  ...
  it("employment history reports what changed between job records", async () => {
    const out = await bambooEmploymentHistory.handler(
      { name: "Emmanuel Castro" },
      fakeCtx(),
    );
    expect(out.found).toBe(true);
    ...
    expect(out.jobChanges[1]?.whatChanged).toEqual(
      expect.arrayContaining(["client", "manager"]),
    );
  ...
  it("recently changed joins the delta feed to real names", async () => {
    ...
    expect(out.changes.find((c) => c.name === "Emmanuel Castro")?.change).toBe(
      "Updated",
    );
  ...
  it("who's out separates people's leave from company holidays", async () => {
    const out = await bambooWhosOut.handler(
  ...
  it("time-off balances come back per policy", async () => {
    const out = await bambooTimeOffBalance.handler(
      { name: "Emmanuel Castro" },
      fakeCtx(),
    );
    expect(out.found).toBe(true);
  ...
  it("lists the projects someone can log time against", async () => {
    const out = await bambooEmployeeProjects.handler(
      { name: "Emmanuel Castro" },
      fakeCtx(),
    );
    expect(out.projects).toEqual([
      { name: "Click Call Sell", hasTasks: false },
    ]);
  });
  ...
  it("documents are listed, flagged as payslips, and never opened", async () => {
    ...
    expect(
      out.documents.find((d) => d.name === "Signed NDA.pdf")?.visibleToEmployee,
    ).toBe(false);
  ...
  it("the field catalogue finds bill rate and names the restricted fields", async () => {
    ...
    expect(out.fields.map((f) => f.name)).not.toContain(
      "Bill Rate - Currency code",
    );
    // Bill Rate has no alias — it is one of Cortex's custom fields.
...
describe("BambooHR tools — a rejected key and other upstream failures", () => {
  it("a 401 blames the key, not the user, and never leaks it", async () => {
    server.use(
      http.post(REPORTS, () => new HttpResponse(null, { status: 401 })),
    );
    const out = await bambooListEmployees.handler(
      { status: "active", limit: 50 },
      fakeCtx(),
  ...
  it("a 403 explains it is a permission problem an HR admin can fix", async () => {
    server.use(
      http.get(META_FIELDS, () => new HttpResponse(null, { status: 403 })),
    );
    const out = await bambooDescribeFields.handler(
      { includeTables: true, limit: 80 },
      fakeCtx(),
  ...
  it("a 429 comes back as a sentence, not an exception", async () => {
    server.use(
      http.get(WHOS_OUT, () => new HttpResponse(null, { status: 429 })),
    );
  ...
  it("a 500 is described as their side, not ours", async () => {
    server.use(
      http.post(REPORTS, () => new HttpResponse(null, { status: 500 })),
    );
    const out = await bambooHeadcount.handler(
...
describe("BambooHR tools — security classification", () => {
  ...
  it("lets a scheduled headcount or who-is-out run unattended", () => {
    expect(
      decide(at("bamboo.headcount", { groupBy: "client" }, "schedule")),
    ).toBe("allow");
