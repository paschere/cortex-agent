 ...
 * against the `gateway.php/zipdev/v1` base. Inventing a second dialect against
...
export const DEFAULT_BAMBOO_BASE = 'https://api.bamboohr.com/api/gateway.php/zipdev/v1';
...
export async function bambooFetch<T>(
  ...
  if (!res.ok) {
    ...
    return { ok: false, configured: true, reason: describeHttpFailure(res.status) };
  }
  ...
  try {
    ...
    return {
      ok: false,
      configured: true,
      reason: 'BambooHR answered with something I could not read. Worth trying again in a moment.',
    };
  }
}
