...
/**
 * What BambooHR actually tracks about the people at the company.
 *
 * A live instance carries ~294 fields and 33 historical tables, most of them
...
const fieldSchema = z.object({
  ...
  /** True when the company added it rather than BambooHR shipping it. */
  custom: z.boolean(),
...
export const bambooDescribeFields = registerTool({
  ...
  description:
    'List what BambooHR records about the people at the company — every field on an employee record and every historical table (job history, compensation history, bill rate history, visas, assets, education and so on), including the custom ones the company added. Use it to answer "do we track X in BambooHR?" before assuming something is or is not recorded. Returns field names only, never anybody\'s values.',
