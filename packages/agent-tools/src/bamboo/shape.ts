...
export function sourceOf(dataset: string): z.infer<typeof sourceSchema> {
  return { provider: 'BambooHR', dataset, retrievedAt: new Date().toISOString() };
}
...
export function failureStatus(f: BambooFailure): { configured: boolean; reason: string } {
  return { configured: f.configured, reason: f.reason };
}

// ---------------------------------------------------------------------------
// Zipdev's BambooHR instance: the field ids that have no alias
// ---------------------------------------------------------------------------
 ...
 * and `/meta/tables`; they are Zipdev-specific and would differ in another
 * BambooHR account.
 *
 ...
 * matters because the payroll app only ever syncs `payRate` — what Zipdev PAYS
...
export const PAYSTUB_YEAR_CATEGORIES: Record<number, number> = { 2025: 29, 2026: 32 };
...
export function parseMoney(raw: unknown): Money {
  ...
  if (typeof raw === 'number') {
    return { amount: Number.isFinite(raw) ? raw : null, currency: null, display: null };
  }
...
/**
 * In Zipdev's instance the BambooHR "department" field holds the CLIENT the
 ...
 * Zipdev, LatAm Staff). Projecting `department` under a `client` label would be
...
export const employeeSchema = z.object({
  ...
  /** BambooHR's "Department" — at Zipdev this is the client or project name. */
 ...
 *
 * `payRate` is what Zipdev PAYS the person — a cost.
 * `billRate` is what Zipdev CHARGES the client for them — revenue.
 *
