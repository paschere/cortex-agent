...
function bodyHtml(body: string): string {
  ...
  for (const line of lines) {
    ...
    if (line.trim())
      out.push(`<p style="margin:0 0 10px;">${inlineHtml(line)}</p>`);
  }
...
async function attempt<T>(
  ...
  try {
    ...
    ctx.logger.warn(
      { err: message, what },
      "meetings.prepare_briefing: lookup failed",
    );
...
async function resolveMeeting(
  ...
  const timeMin = new Date(
    base - (input.startTime ? 2 : 0) * 86_400_000,
  ).toISOString();
  const timeMax = new Date(
    base + (input.startTime ? 2 : 14) * 86_400_000,
  ).toISOString();
  ...
  const scored = candidates
    .map((m) => {
      const titleScore =
        needle && m.title.toLowerCase().includes(needle) ? 0 : 1;
...
export const meetingsPrepareBriefing = registerTool({
  ...
  inputSchema: z
    .object({
      eventId: z
        .string()
        .optional()
        .describe("Calendar entry id for the meeting"),
      title: z
        .string()
        .optional()
        .describe("Meeting title, when the calendar id is unknown"),
      startTime: z
        .string()
        .optional()
        .describe(
          "Roughly when the meeting starts (ISO), to disambiguate by title",
        ),
      ...
      includeTranscript: z
        ...
        .default(true)
        .describe(
          "Attach an excerpt of the last recorded call with the same people",
        ),
  ...
  handler: async (input, ctx) => {
    ...
    if (type === "interview") {
      ...
      if (!lookupKey) {
        ...
        const search = await attempt(
          ctx,
          gaps,
          "The candidate's record in Workable",
          async () =>
        ...
        if (top) {
          ...
          const otherApps = hits
            ...
            .map(
              (h) =>
                `${h.jobTitle ?? "another role"} (${h.stage ?? "stage unknown"})`,
            )
            .slice(0, 4);
          ...
          const body = [
            ...
            top.updatedAt
              ? `**Last movement:** ${freshnessOf(top.updatedAt)}`
              : "",
          ...
          if (top.jobShortcode) {
            ...
            if (job.value?.job) {
              ...
              sections.push({
                ...
                body: [
                  `**${job.value.job.title}**`,
                  job.value.job.department
                    ? `**Team:** ${job.value.job.department}`
                    : "",
                  job.value.job.state
                    ? `**Status:** ${job.value.job.state}`
                    : "",
                ]
                  .filter(Boolean)
                  .join("\n"),
    ...
    if (type === "client") {
      ...
      if (!domain) {
        gaps.push(
          "No outside guest is listed, so the account could not be identified.",
        );
      } else {
        const search = await attempt(
          ctx,
          gaps,
          "The company record in HubSpot",
          async () =>
            call<{
              results: Array<{
                id: string;
        ...
        if (company) {
          const full = await attempt(
            ctx,
            gaps,
            "The account detail and open deals",
            async () =>
              call<{
                name: string | null;
                domain: string | null;
          ...
          const contact = await attempt(
            ...
            async () =>
              call<{
                ...
                { query: primaryExternal.email, limit: 1 },
                ctx,
              ),
          );
      ...
      if (last) {
        ...
        sections.push({
          ...
          body: [
            ...
            last.participants.length
              ? `**Who was there:** ${last.participants.join(", ")}`
              : "",
            "",
        ...
        talkingPoints.push(
          "Pick up the thread from the last call before opening anything new.",
        );
    ...
    const markdown = [
      ...
      ...sections.flatMap((s) => [
        ...
        s.body,
        s.sourceRefs.length
          ? `_Source: ${s.sourceRefs.map((r) => `[${r}]`).join(" ")}_`
          : "",
        "",
      ]),
      ...
      "",
      ...(gaps.length
        ? ["## What could not be found", ...gaps.map((g) => `- ${g}`), ""]
        : []),
      "## Sources",
      ...sourceList.map(
        (s) => `[${s.ref}] ${s.label} — ${s.detail} · ${s.freshness}`,
      ),
    ]
