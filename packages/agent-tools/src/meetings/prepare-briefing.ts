...
async function resolveMeeting(
  ctx: ToolContext,
  input: { eventId?: string; title?: string; startTime?: string; calendarId?: string },
...
export const meetingsPrepareBriefing = registerTool({
  ...
  outputSchema: z.object({
    event: z.object({
      ...
      attendees: z.array(
        z.object({ email: z.string(), name: z.string().nullable(), external: z.boolean() }),
      ),
  ...
  handler: async (input, ctx) => {
    ...
    sections.push({
      ...
      body: [
        ...
        m.conferenceLink ? `**Join:** ${m.conferenceLink}` : m.location ? `**Where:** ${m.location}` : '',
      ...
      if (!lookupKey) {
        ...
        if (top) {
          ...
          const scored = await attempt(
            ...
            gaps,
            'Scoring history',
            async () => call<{ markdown: string }>('recruit.get_candidate', { candidateId: top.id }, ctx),
            { silent: true },
          );
          ...
          const body = [
            ...
            detail.value?.candidate.source ? `**Came in via:** ${detail.value.candidate.source}` : '',
          ...
          sections.push({ key: 'candidate', title: 'The candidate', body, sourceRefs: refs });
          ...
          if (top.jobShortcode) {
            const job = await attempt(
              ...
              async () =>
                call<{
                  job: { title: string; state: string | null; department: string | null };
    ...
    if (type === 'client') {
      ...
      if (!domain) {
        ...
        const search = await attempt(ctx, gaps, 'The company record in HubSpot', async () =>
          call<{ results: Array<{ id: string; name: string | null; domain: string | null }> }>(
            'hubspot.search_companies',
            { query: domain, limit: 3 },
            ctx,
          ),
        );
        ...
        if (company) {
          ...
          sections.push({
            ...
            body: [
              ...
              c?.numEmployees != null ? `**Size:** ${c.numEmployees.toLocaleString()} employees` : '',
              deals.length
                ? `**Open deals:**\n${deals
                    .map(
                      (d) =>
        ...
        if (primaryExternal?.email) {
          ...
          if (person) {
            const timeline = await attempt(ctx, gaps, 'Recent activity with this contact', async () =>
              call<{
                results: Array<{ type: string; subject: string | null; createdAt: string | null }>;
              }>('hubspot.get_contact_timeline', { contactId: person.id, days: 90, limit: 10 }, ctx),
            );
    ...
    const kb = await attempt(ctx, gaps, "Zipdev's Knowledge Base", async () =>
      call<{
        sources: Array<{ ref: number; documentTitle: string; excerpts: string[] }>;
        contextBlock: string;
      }>(
        'kb.context',
        {
    ...
    if (kbSources.length) {
      const ref = sources.add(
        'Zipdev Knowledge Base',
        `${kbSources.length} document(s) on "${m.title}"`,
        'checked just now',
      );
      sections.push({
        key: 'knowledge',
        title: 'What Zipdev already knows',
        body: kbSources
          .slice(0, 4)
          .map((s) => `- **${s.documentTitle}** — ${(s.excerpts[0] ?? '').replace(/\s+/g, ' ').slice(0, 320)}…`)
          .join('\n'),
        sourceRefs: [ref],
      });
    } else if (kb.ok) {
    ...
    if (input.includeTranscript !== false) {
      ...
      const transcripts = await attempt(ctx, gaps, 'The transcript of the previous call', async () =>
        runTool(
    ...
    if (type === 'interview') {
      ...
      talkingPoints.push('Confirm the purpose of the meeting at the top so the time is well spent.');
    }
    if (gaps.length) {
      talkingPoints.push(
    ...
    const whyThisMatters =
      type === 'interview'
        ? `This is an interview${primaryExternal ? ` with ${primaryExternal.name ?? primaryExternal.email}` : ''}. Everything below is what Zipdev already knows about them, so the call can go straight to what is not on paper.`
        : type === 'client'
    ...
    return {
      event: {
        ...
        attendees: guests.map((a) => ({ email: a.email, name: a.name, external: a.external })),
