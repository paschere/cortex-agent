...
export const listRecent = registerTool({
  ...
  handler: async (input, ctx) => {
    ...
    const meta = buildMeta({
      endpoint: 'presentation_files (Zipdev agent DB)',
      ...
      provenance: {
        'candidateName, filename, sizeBytes, createdAt':
          'Zipdev agent DB — recorded when the PDF was prepared',
        downloads: 'Zipdev agent DB — counted on every click of the download link',
        'the document itself': 'AI-written by the Zipdev matcher at the time it was prepared',
      },
      dataQuality: [
