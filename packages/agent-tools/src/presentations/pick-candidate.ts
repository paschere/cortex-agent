...
function renderMarkdown(rows: any[], jobLabel: string, meta: ToolMeta): string {
  ...
  const lines: string[] = [];
  lines.push(`**Candidates on ${jobLabel}** — ${rows.length} shown, best match first.`);
  lines.push('');
...
export const pickCandidate = registerTool({
  ...
  description:
    ...
    'PROVENANCE: candidates and stages come from the Zipdev matcher; match scores are Zipdev AI output, not an ATS field; presentation drafts are AI-written unless a recruiter has edited them.',
  inputSchema: z.object({
    jobId: z.string().min(1).describe('The requisition to list candidates for.'),
    stage: z
      .string()
      .optional()
      .describe('Optional pipeline stage filter, e.g. CLIENT_MANAGER or RECRUITER_INTERVIEW.'),
    ...
    includePresentationStatus: z
      .boolean()
      .default(true)
      .describe('Set false to skip the per-candidate presentation lookup and answer faster.'),
  }),
  ...
  handler: async (input) => {
    ...
    if (lean.available) {
      ...
      if (input.minScore != null) {
        all = all.filter(
          (c) => typeof c?.matchScore === 'number' && c.matchScore >= (input.minScore as number),
        );
      }
    ...
    const withPresentation = candidates.filter((c) => c.presentation.state === 'exists').length;
    ...
    return {
      job: job ? { id: job.id ?? input.jobId, title: job.title ?? null } : { id: input.jobId },
      candidates,
      meta,
