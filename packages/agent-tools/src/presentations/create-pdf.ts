...
export const createPdf = registerTool({
  ...
  description: [
    ...
    `What it does, in order: if the candidate has no presentation yet (or regenerate is set) it has one written first, renders it to a PDF on Zipdev letterhead, stores it, and returns a download link on our own domain that expires in ${DEFAULT_EXPIRY_DAYS} days.`,
    ...
    'PROVENANCE: the text is AI-written from the matcher profile and the latest scoring insights; the PDF is the Zipdev-format render of exactly that stored draft, pinned at the version reported here.',
  ].join(' '),
  inputSchema: z.object({
    candidateId: z.string().min(1).describe('The person the presentation is about.'),
    jobId: z
      .string()
      .optional()
      .describe('The requisition this presentation is for, recorded alongside the file.'),
    regenerate: z
      .boolean()
      .default(false)
      .describe(
  ...
  handler: async (input, ctx) => {
    ...
    const humanReviewed = !!presentation?.lastEditedBy && presentation.lastEditedBy !== 'AI';
    ...
    const meta = buildMeta({
      ...
      provenance: {
        ...
        'PDF layout': `${SOURCE.matcher} — rendered by the matcher's Zipdev-letterhead export`,
        'download link': 'Zipdev agent — stored in Zipdev private storage, link expires',
      },
      dataQuality: [
        humanReviewed
