...
describe("screenMemory", () => {
  ...
  it("refuses credentials, labelled or not", () => {
    expect(screenMemory("Their API key is stored in the vault.").reason).toBe(
      "credential",
    );
    expect(
      screenMemory("Use sk-abcd1234efgh5678ijkl9012 for that.").reason,
    ).toBe("credential");
    expect(
      screenMemory("Token: ghp_ABCDEFGHIJKLMNOPQRSTUVWXYZ012345").reason,
    ).toBe("credential");
  });

  it("refuses a pay figure attached to a pay word", () => {
    expect(screenMemory("María's salary is 4,500 USD a month.").reason).toBe(
      "compensation",
    );
    expect(screenMemory("Su sueldo es de 12.000.000 COP.").reason).toBe(
      "compensation",
    );
  });
...
describe("behaviouralCandidates", () => {
  ...
  it("says nothing when there is nothing to count", () => {
    ...
    expect(
      behaviouralCandidates(
        [{ tool_id: "kb.search", status: "ok", created_at: at(1, 12) }],
        "UTC",
      ),
    ).toEqual([]);
  });
  ...
  it("flags a family that keeps failing, once", () => {
    ...
    const failures = behaviouralCandidates(rows, "UTC").filter(
      (c) => c.kind === "instruction",
    );
...
describe("usableCandidates", () => {
  ...
  it("drops what the person already has, in any status", () => {
    const out = usableCandidates(
      [
        candidate("Prefers costs in USD."),
        candidate("Owns the Growth pipeline."),
      ],
  ...
  it("drops anything the sensitivity screen refuses, even if the model liked it", () => {
    const out = usableCandidates(
      [candidate("Their salary is 5,000 USD.")],
      [],
      5,
    );
    expect(out).toEqual([]);
  ...
  it("respects the per-run cap", () => {
    const many = Array.from({ length: 10 }, (_, i) =>
      candidate(`Fact number ${i} about them.`),
    );
