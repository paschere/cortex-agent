...
export const cortexRemember = registerTool({
  id: 'cortex.remember',
  description:
    'Remember something about the person you are talking to, so you stop needing to be told it. Use this when they tell you a standing preference, a rule to follow from now on, what one of their words means, or a stable fact about them and their work — and only when it should hold beyond this conversation. Write it as one short sentence in the third person, the way you would want to read it later: "prefers costs in USD", not "the user said USD". After saving an instruction, say in one clause what you will do differently next ("el jueves igual, el lunes no") — do not recap the sentence they just said. This is NOT for storing documents, notes or work products (save those to a Brain Knowledge space), NOT for company-wide facts everyone should know (those belong in a company space, or only the person who told you benefits), and NOT for anything sensitive — no passwords or keys, no pay figures, no emails or phone numbers.',
