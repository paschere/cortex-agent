...
function fixture(): Row[] {
  ...
  return [
    { id: 'm-ana-1', user_id: ANA, content: 'Prefers costs in USD.', status: 'active', ...base },
    {
      id: 'm-ana-2',
      user_id: ANA,
      content: 'Calls tpp.zipdev.com "the matcher".',
      status: 'active',
      ...base,
    },
    {
...
describe('memory isolation', () => {
  it("never returns another person's memories", async () => {
    ...
    expect(ana.map((m) => m.content)).toEqual([
      ...
      'Calls tpp.zipdev.com "the matcher".',
    ]);
    // The whole point.
  ...
  it('writes land on the caller, so one person cannot plant a memory on another', async () => {
    const { db, rows } = makeDb(fixture());
    await rememberMemory(db, { userId: ANA, content: 'Always answers in Spanish.' });
