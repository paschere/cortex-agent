 ...
 * primitives in ./tools.ts and follow the matcher's proven access
...
export const workableJobCandidatesSummary = registerTool({
  ...
  handler: async (input, ctx) => {
    ...
    const jobTitle: string | null = raw.find((c) => c.job?.title)?.job?.title ?? null;
    ...
    for (const c of raw) {
      ...
      const name = (
        (c.name ?? [c.firstname, c.lastname].filter(Boolean).join(' ')) as string
      ).slice(0, 80);
