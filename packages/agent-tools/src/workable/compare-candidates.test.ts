  ...
  it("compares head-to-head with one LLM call: winner, trade-offs, recommendation", async () => {
    ...
    generateObjectMock.mockResolvedValue({
      object: {
        perCandidate: [
          {
            ...
            fitSummary:
              "Full core-stack coverage and already in technical interview.",
        ...
        tradeoffs: [
          "Maria brings React+AWS; Juan brings longer backend tenure",
        ],
    ...
    expect(workableCompareCandidates.outputSchema.safeParse(out).success).toBe(
      true,
    );
    ...
    expect(llmArgs.prompt).toContain(
      "testgorilla (verified assessment): React: 88",
    );
    expect(llmArgs.system).toContain(
      "TestGorilla lines are verified assessment results",
    );
