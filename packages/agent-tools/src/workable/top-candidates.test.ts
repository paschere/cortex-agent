...
const JOB = {
  ...
  state: 'published',
  requirements: '<p>We need React, Node.js, TypeScript and AWS. GraphQL is a plus.</p>',
...
function mockWorkable(candidates: any[], details: Record<string, any>) {
  return vi.fn(async (url: string | URL | Request) => {
    ...
    if (href.includes('/candidates?')) {
      return new Response(JSON.stringify({ candidates, paging: {} }), { status: 200 });
    }
...
describe('workable.top_candidates', () => {
  ...
  it('merges the single batched LLM evaluation with the deterministic evidence', async () => {
    const candidates = [
      listRow('weak'),
      listRow('strong', { stage: 'Technical Interview', updated_at: '2026-07-20T00:00:00Z' }),
      listRow('dq', { disqualified: true }),
    ];
    globalThis.fetch = mockWorkable(candidates, STRONG_DETAILS) as unknown as typeof fetch;
    llmAnswers({
      ranking: [
        {
          candidateId: 'strong',
          ...
          strengths: ['React + Node.js + AWS verified on profile', '8+ years in role'],
    ...
    expect(workableTopCandidates.outputSchema.safeParse(out).success).toBe(true);
  ...
  it('scores coverage of mustHaveSkills and names what is missing', async () => {
    ...
    globalThis.fetch = mockWorkable(candidates, details) as unknown as typeof fetch;
    llmAnswers({
      ranking: [
        {
          candidateId: 'partial',
  ...
  it('caps hydration, prioritizes advanced stages, and discloses who was skipped', async () => {
    const candidates = [
      ...
      listRow('interview', { stage: 'Client Interview', updated_at: '2026-07-25T00:00:00Z' }),
    ];
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const details: Record<string, any> = { interview: detail('interview', { skills: ['React'] }) };
    for (let i = 0; i < 8; i++) details[`applied-${i}`] = detail(`applied-${i}`);
    ...
    expect(out.meta.dataQuality.join(' ')).toContain('did not get a deep profile look');
