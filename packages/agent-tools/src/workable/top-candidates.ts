  ...
  ranking: z.array(
    z.object({
      ...
      why: z
        .string()
        .describe(
          "1-2 sentences: why this candidate earns this rank, citing the data",
        ),
...
async function rankWithLlm(
  ...
  const system = [
    "You are a senior technical recruiter at Cortex evaluating candidates for one job.",
...
export const workableTopCandidates = registerTool({
  ...
  inputSchema: z.object({
    shortcode: z
      .string()
      .min(1)
      .describe("Workable job shortcode, e.g. from workable.list_jobs"),
    ...
    mustHaveSkills: z
      ...
      .optional()
      .describe(
        "Skills the role truly requires; when set, ranking scores coverage of this list",
      ),
    stage: z
      .string()
      .optional()
      .describe("Only consider candidates currently in this stage"),
    maxProfiles: z
      .number()
      .int()
      .min(5)
      ...
      .describe(
        "How many full profiles to load for deep ranking (each costs one ATS call)",
      ),
  }),
  ...
  handler: async (input, ctx) => {
    ...
    const tg = await fetchTestGorillaSignals(
      collected.map(({ detail, row }) =>
        String(detail?.email ?? row?.email ?? ""),
      ),
    );
    const hydrated: EvidenceCandidate[] = collected.map(({ detail, row }) =>
      buildEvidence(
        ...
        mustHaves,
        tg.byEmail.get(
          String(detail?.email ?? row?.email ?? "").toLowerCase(),
        ) ?? null,
      ),
    );
    ...
    const llmPool = preRanked.slice(
      0,
      Math.min(MAX_LLM_CANDIDATES, preRanked.length),
    );
    ...
    if (profileErrors > 0) {
      dataQuality.push(
        `${profileErrors} profile(s) failed to load and were left out.`,
      );
    }
    if (tg.note) {
    ...
    dataQuality.push(
      llm
        ? "Scores, verdicts, strengths and concerns are Cortex AI output computed just now over live Workable data — not ATS fields and not client feedback. `evidence` lines are deterministic and verifiable in the profile."
