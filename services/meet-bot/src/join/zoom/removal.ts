...
function redactZoomUrl(value: string): string {
  try {
...
export function startZoomRemovalMonitor(
  ...
  const triggerRemoval = async (reason: string) => {
    ...
    log(`[Zoom Web] Current URL at removal: ${redactZoomUrl(page.url())}`);
  ...
  const onNavigated = (frame: any) => {
    ...
    if (Date.now() - joinedAtMs < GRACE_PERIOD_MS) {
      log(`[Zoom Web] Ignoring navigation during grace period (${elapsed}s after join): ${redactZoomUrl(url)}`);
      return;
    }
    ...
    if (isZoomAudioInitUrl(url)) {
      log(`[Zoom Web] Ignoring audio-init redirect URL (${elapsed}s after join): ${redactZoomUrl(url)}`);
      return;
    }
    ...
    if (!/zoom\.(us|com|eu|com\.cn|com\.br|com\.au|de|fr|jp|ca|co\.uk)\b/.test(url)) {
      triggerRemoval(`Navigation away from Zoom domain: ${redactZoomUrl(url)}`);
    } else if (url.includes('/signin') || url.includes('/login')) {
      // Explicit sign-in redirect — meeting definitely ended
      triggerRemoval(`Navigation to Zoom sign-in: ${redactZoomUrl(url)}`);
    } else if (url.includes('/wc/') && !url.includes('/meeting')) {
      // Non-meeting /wc/ URL — but only if it's not a known init pattern
      log(`[Zoom Web] Suspicious non-meeting /wc/ URL (${elapsed}s after join): ${redactZoomUrl(url)} — deferring to polling`);
  ...
  const poll = async () => {
    ...
    try {
      ...
      if (!leaveVisible) {
        ...
        const elapsed = ((Date.now() - joinedAtMs) / 1000).toFixed(1);
        log(`[Zoom Web] Leave button miss #${consecutiveLeaveButtonMisses} (${elapsed}s after join) — URL: ${redactZoomUrl(url)}, title: "${title}"`);
        ...
        if (Date.now() - joinedAtMs < GRACE_PERIOD_MS || isZoomAudioInitUrl(url)) {
          ...
          if (url && !url.startsWith('about:') && !/zoom\.(us|com|eu|com\.cn|com\.br|com\.au|de|fr|jp|ca|co\.uk)\b/.test(url)) {
            await triggerRemoval(`Leave button gone and URL left Zoom domain: ${redactZoomUrl(url)}`);
            return;
          }
          // Redirected to sign-in — immediate exit
          if (url.includes('/signin') || url.includes('/login')) {
            await triggerRemoval(`Leave button gone and redirected to sign-in: ${redactZoomUrl(url)}`);
            return;
          }
          ...
          if (consecutiveLeaveButtonMisses >= LEAVE_BUTTON_MISS_THRESHOLD) {
            ...
            if (url.includes('/wc/') && !url.includes('/meeting')) {
              await triggerRemoval(`Leave button gone ${consecutiveLeaveButtonMisses}x and URL is non-meeting: ${redactZoomUrl(url)}`);
              return;
            }
            // Error page or blank
