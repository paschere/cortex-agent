...
export function buildZoomWebClientUrl(meetingUrl: string): string {
  try {
    ...
    if (isCanonicalZoomHost) {
      ...
      if (!meetingId) {
        throw new Error('Cannot extract meeting ID from Zoom URL');
      }
...
export async function joinZoomMeeting(
  ...
  log(`[Zoom Web] Navigating to web client: ${redactZoomUrl(webClientUrl)}`);
  ...
  try {
    ...
    if (botConfig.cameraEnabled) {
      // "Start Video" means currently off → click to publish the virtual camera.
      if (videoAriaLabel === 'Start Video') {
        await videoBtn.click();
        log('[Zoom Web] Started video in preview (virtual camera)');
      } else {
        log('[Zoom Web] Preview video already on (virtual camera)');
      }
