...
/** Serialize XTEST pointer use: one Xvfb display cannot drive two Chromes at once. */
let x11Lock: Promise<unknown> = Promise.resolve();
function withX11Lock<T>(fn: () => Promise<T>): Promise<T> {
  const next = x11Lock.then(fn, fn);
  x11Lock = next.then(() => undefined, () => undefined);
  return next;
}
...
export class HumanizedInteractor {
  ...
  async navigateAndClick(page: Page, handle: ElementHandle<Element>): Promise<void> {
    return withX11Lock(() => this.navigateAndClickUnlocked(page, handle));
  }

  private async navigateAndClickUnlocked(page: Page, handle: ElementHandle<Element>): Promise<void> {
  ...
  async fillField(page: Page, handle: ElementHandle<Element>, text: string): Promise<void> {
    await withX11Lock(async () => {
      await this.navigateAndClickUnlocked(page, handle);
