...
export function findLobbyPrimaryCta(opts: LobbyCtaScanOptions): LobbyCtaScan {
  ...
  for (let i = 0; i < buttons.length; i++) {
    ...
    if (!view) continue;
    const style = view.getComputedStyle(btn);
...
export async function joinGoogleMeeting(
  ...
  // Interstitials Vexa doesn't always hit ("use the browser", "continue as guest").
  // Dismissing them before the lobby CTA wait avoids a 60s miss on the join button.
  ...
  for (const label of DISMISS_LABELS) {
    try {
      ...
      if (handle) await clickHandle(handle, label);
    } catch { /* best-effort */ }
  }
