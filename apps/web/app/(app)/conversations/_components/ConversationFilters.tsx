...
export function ConversationFilters({ surface, q }: { surface: SurfaceFilter; q: string }) {
  return (
    <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
      <div
        className="inline-flex flex-wrap items-center gap-0.5 rounded-card border border-border bg-surface p-0.5"
        role="tablist"
      ...
      <form method="get" action="/conversations" className="flex items-center gap-2">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-56 rounded-card border border-border bg-surface py-1.5 pl-8 pr-3 text-[12.5px] text-ink transition-colors placeholder:text-ink-faint focus:border-border-strong focus:outline-none"
          />
        </div>
        {q && (
          <Link
            ...
            className="rounded-card p-1.5 text-ink-faint transition-colors hover:bg-surface-2 hover:text-ink"
          >
