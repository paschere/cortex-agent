  ...
  return (
    <Panel className="animate-rise flex h-full flex-col overflow-hidden">
      <div className="flex items-center justify-between gap-3 px-4 py-3">
        ...
        <Link
          href={href}
          className="group inline-flex items-center gap-1 text-[11px] font-semibold text-primary hover:text-primary-strong"
        >
          La jornada completa
      ...
      <p
        className={clsx(
          'px-4 pt-3 text-[13px] font-semibold leading-snug',
      ...
      {hidden > 0 && (
        <div className="px-4 pb-2">
          <Link href={href} className="text-[11.5px] font-semibold text-primary hover:underline">
        ...
        <div className="mt-auto border-t border-border px-4 py-2.5">
          ...
          <ul className="space-y-1">
            {journal.lingering.map((line) => (
              <li key={line.id} className="flex items-start gap-2 text-[12px] leading-snug">
...
export function JournalRow({ line, day }: { line: JournalLine; day: string | null }) {
  ...
  const body = (
    <div className="flex items-start gap-2.5">
      <span className="tabular mt-[1px] w-[38px] shrink-0 text-[11.5px] font-semibold text-ink-faint">
        {line.clock}
      </span>
  ...
  return (
    <li>
      {day && (
        <div className="px-2 pb-1 pt-2 text-[10.5px] font-bold uppercase tracking-wider text-ink-faint first:pt-0">
          {day}
        </div>
      )}
      {line.href ? (
...
export function JournalGaps({ gaps }: { gaps: string[] }) {
  ...
  return (
    <div className="mt-auto border-t border-border px-4 py-2.5">
      <ul className="space-y-1">
        {gaps.map((gap) => (
          <li key={gap} className="flex items-start gap-1.5 text-[11.5px] leading-snug text-rose">
...
export function JournalQuiet({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-2.5 px-4 py-6 text-[12.5px] text-ink-muted">
