 ...
 * from Slack, or from an email. None of those carry a Cortex session cookie, so
...
export async function GET(
  ...
  if (error) {
    ...
    return problem(
      "Could not look up this download right now. Please try again.",
      500,
    );
  }
  if (!data) {
  ...
  if (downloadError || !file) {
    console.error(
      "[files/presentation] storage read failed:",
  ...
  void sb
    ...
    .then(({ error: bumpError }) => {
      if (bumpError)
        console.error(
          "[files/presentation] counter bump failed:",
          bumpError.message,
        );
    });
