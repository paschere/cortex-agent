...
/**
 * Why each gated tool is gated — surfaced to the model so it can EXPLAIN the
 * stakes to the user before asking for approval. Tools without an entry get
 * a generated reason from their family/verb.
 */
const CONFIRMATION_NOTES: Record<string, string> = {
  ...
  'hubspot.create_contact': 'Creates a permanent CRM record the whole sales team will see and rely on.',
  'hubspot.create_deal': 'Creates a deal in the sales pipeline — it will appear in forecasts and reports.',
  'hubspot.update_deal':
...
function confirmationReason(toolId: string): string {
