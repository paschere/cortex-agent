...
export function isGoogleChatWebhookUrl(raw: string): boolean {
  ...
  if (!/^\/v1\/spaces\/[A-Za-z0-9_-]+\/messages$/.test(parsed.pathname))
    return false;
  return Boolean(
    parsed.searchParams.get("key") && parsed.searchParams.get("token"),
  );
}
...
/** Short, curated list — the zones most teams actually work across. */
...
export const PreferencesBody = z
  ...
  .partial()
  .refine(
    (b) => !b.chatWebhookUrl || isGoogleChatWebhookUrl(b.chatWebhookUrl),
    {
      message:
