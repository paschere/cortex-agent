...
interface Body {
  frames?: { base64?: string; mimeType?: string; atMs?: number }[];
  hint?: string;
}
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const frames: Frame[] = (body.frames ?? [])
    .filter((f): f is { base64: string; mimeType: string; atMs: number } =>
      Boolean(f.base64 && f.mimeType),
    )
    .slice(0, MAX_FRAMES)
    .map((f) => ({ base64: f.base64, mimeType: f.mimeType, atMs: f.atMs ?? 0 }));
