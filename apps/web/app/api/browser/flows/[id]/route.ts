...
/**
 * Cambiar qué produce el trámite y dónde llega.
 *
 * Separate from the create path because the decision outlives the recording: a
 * trámite taught in March and scheduled in August is exactly when somebody
 * discovers they want it in their inbox, and re-teaching an errand to change
 * where its answer goes would be absurd.
 *
 * Only the four columns of migration 0093 are writable here. The steps, the
 * status and the credential binding are the engine's, and a PATCH that could
 * reach them would be a way to edit a proven flow without re-proving it.
 */
