...
export function renderRoutineResultEmail(input: RoutineResultEmailInput): RenderedEmail {
  ...
  const stats = [
    { label: 'Ran at', value: `${formatMoment(input.ranAt, zone)}${zone === 'UTC' ? ' UTC' : ''}` },
    {
      label: 'Took',
  ...
  const build = (reportBudget: number): string => {
    ...
    const body = input.ok
      ? [
          ...
          clamped.truncated
            ? fineprint('This report was shortened for email. See the full result in Zipdev OS.')
            : '',
          detailUrl ? button({ href: detailUrl, label: 'Open in Zipdev OS' }) : '',
        ]
      : [
          statRow(stats),
          calloutBox({
    ...
    return renderEmail({
      ...
      footerNote: `You are on the recipient list for the "${input.jobName}" routine. Change or pause it in Zipdev OS.`,
    });
  };
  ...
  const text = [
    ...
    textReport.truncated ? '\n(Shortened for email — see the full result in Zipdev OS.)' : null,
    '',
    detailUrl ? `Open in Zipdev OS: ${detailUrl}` : null,
  ]
