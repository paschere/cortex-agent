import { button, calloutBox, codeBlock, keyValueTable, statRow, statusPill } from './components';
...
function checkLine(checks: DevTaskEmailCheck[]): string | null {
  ...
  if (pending > 0) return `${passed} of ${checks.length} checks passed, ${pending} still running`;
...
export function renderDevTaskEmail(input: DevTaskEmailInput): RenderedEmail {
  ...
  const build = (budget: number): string => {
    ...
    const body = ok
      ? [
          ...
          input.prUrl ? button({ href: input.prUrl, label: 'Review the pull request' }) : '',
          detailUrl
            ? button({
                href: detailUrl,
                label: 'See the whole run in Zipdev OS',
                tone: 'quiet',
              })
            : '',
        ]
      : [
          calloutBox({ tone: 'danger', title: 'What went wrong', text: reason }),
          stats,
          facts,
          input.errorDetail ? codeBlock(input.errorDetail, { label: 'Technical detail' }) : '',
          detailUrl ? button({ href: detailUrl, label: 'Open the run in Zipdev OS' }) : '',
        ];
  ...
  const text = [
    ...
    '',
    ok && textSummary.markdown ? `What changed:\n\n${textSummary.markdown}` : null,
    !ok && input.errorDetail ? `Technical detail:\n\n${input.errorDetail}` : null,
    '',
