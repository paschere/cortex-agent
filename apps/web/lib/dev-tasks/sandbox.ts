...
export async function commitAll(sandbox: Sandbox, message: string): Promise<void> {
  const added = await run(sandbox, { cmd: 'git', args: ['add', '-A'], cwd: REPO_ROOT });
...
export async function pushBranch(
  sandbox: Sandbox,
  params: { branch: string; defaultBranch: string; cloneUrl: string; token: string },
): Promise<void> {
  const argv = assertPushable({ branch: params.branch, defaultBranch: params.defaultBranch });
...
export async function stopSandbox(taskId: string, repo?: RepoRef): Promise<void> {
  try {
    const sandbox = await Sandbox.get({ name: sandboxNameForTask(taskId), resume: false });
    await sandbox.stop();
  } catch (err) {
