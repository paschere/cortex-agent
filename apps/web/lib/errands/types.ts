/**
 * Shapes shared by the errand engine, its HTTP surface and the screens.
 *
 * Deliberately free of `server-only` imports and of anything that reaches
 * `@cortex/agent-tools`: the detail page is a client component and importing
 * the tools barrel from `'use client'` drags in `node:dns` and breaks the
 * production build while typecheck and tests stay green. See
 * lib/commitments-shape.ts for the full account of that trap.
 */
...
export const ERRAND_COLUMNS =
  ...
  'current_run_id, findings, deliverable, sources, closing_note, last_heartbeat_at, claimed_at, ' +
