...
/**
 * El cuerpo, extraído a la firma de la cola nueva; la concurrencia (5) la fija
 * el manifiesto del worker, no esta función.
 */
export const scheduleRunJob: JobHandler = async ({ event, step }) => {
...
export const scheduleRun = inngest.createFunction(
  ...
  async (ctx) => scheduleRunJob(ctx as unknown as JobContext),
);
